...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Houston"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Houston'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Elias" age=30 height=5.3')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Elias'", cout.getvalue().strip())
            self.assertIn("'age': 30", cout.getvalue().strip())
            self.assertIn("'height': 5.3", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="elaman@yahoo.com" password="1"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="elaman@yahoo.com", password="1")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('elaman@yahoo.com', result)
            self.assertIn('1', result)
            self.assertIn('elaman@yahoo.com', cout.getvalue())
            self.assertIn('1', cout.getvalue())
            cursor.close()
            dbc.close()
    ...
    def test_db_count(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            cons.onecmd('create State name="Alaska"')
            clear_stream(cout)
